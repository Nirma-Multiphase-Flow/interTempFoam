...
:
    ...
    (
        ...
        readScalar(subDict(phase1Name_).lookup("cp"))
    ),
    ...
    (
        "cp2",
        dimEnergy/dimMass/dimTemperature,
        readScalar(subDict(phase2Name_).lookup("cp"))
    ),
    ...
    (
        "Pr1",
        dimless,
        readScalar(subDict(phase1Name_).lookup("Pr"))
    ),
    ...
    (
        "Pr2",
        dimless,
        readScalar(subDict(phase2Name_).lookup("Pr"))
    ),
...
{
    return
        alpha1_*rho1()*nuModel1_->nu()*cp1_/Pr1_
      + alpha2_*rho2()*nuModel2_->nu()*cp2_/Pr2_;
}
